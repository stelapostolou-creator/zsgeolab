...
(function (root) {
  ...
  function surface(pts, maxEdge) {
    ...
    /*  ΠΡΟΕΠΙΛΟΓΗ: κανένα φιλτράρισμα. Την περιοχή την ορίζει το ΠΟΛΥΓΩΝΟ,
     *  και ό,τι πέφτει έξω δεν μετριέται ποτέ. Ένα δεύτερο, έμμεσο μάντεμα
     *  για το «πού πάτησε ο χρήστης» απλώς πρόσθετε τρόπους να βγει λάθος:
     *  με μεικτή πυκνότητα — αραιά στο ανοιχτό, πυκνά στη γραμμή αλλαγής
     *  κλίσης, δηλαδή σωστή μέτρηση — πετούσε το 80% του εδάφους.
     *
     *  Το όριο μένει διαθέσιμο για να δοθεί στο χέρι, όταν το πολύγωνο είναι
     *  μεγαλύτερο από τη μέτρηση (π.χ. ένα ΚΑΕΚ που μετρήθηκε μόνο εν μέρει)
     *  και δεν θέλουμε ο τριγωνισμός να γεφυρώσει το κενό. */
    var lim = (maxEdge > 0) ? maxEdge : Infinity;
  ...
  function hull(points, tight) {
    if (!points || points.length < 3) return { err: 'At least 3 points are needed.' };
    var ox = points[0].x, oy = points[0].y;
    var pts = points.map(function (p) { return { x: p.x - ox, y: p.y - oy, z: p.z }; });

    var tris = triangulate(pts);
    if (!tris.length) return { err: 'The points are collinear — no boundary can be formed.' };
    ...
    if (!keep.length) return { err: 'The tightness is too low — nothing is left.' };
    ...
    if (!rings.length) return { err: 'No closed boundary could be formed.' };
  ...
  /** Τα τρίγωνα της επιφάνειας, σε απόλυτες συντεταγμένες — το ψηφιακό μοντέλο. */
  function mesh(points, maxEdge) {
    if (!points || points.length < 3) return { err: 'At least 3 points are needed.' };
    var ox = points[0].x, oy = points[0].y;
    var pts = points.map(function (p) { return { x: p.x - ox, y: p.y - oy, z: p.z }; });
    var all = triangulate(pts);
    if (!all.length) return { err: 'The points are collinear — no surface can be formed.' };
    var lim = (maxEdge > 0) ? maxEdge : Infinity;
    var out = [], dropped = 0;
    all.forEach(function (t) {
      if (longestEdge(t, pts) > lim) { dropped++; return; }
      out.push(t.map(function (i) {
        return { x: pts[i].x + ox, y: pts[i].y + oy, z: pts[i].z };
      }));
    });
    return { tris: out, dropped: dropped, points: points.length };
  }

  /**
   *  Πού τέμνει το οριζόντιο επίπεδο «z» το τρίγωνο; Δύο σημεία, ή τίποτα.
   *
   *  Κορυφή ακριβώς πάνω στη στάθμη μετράει ΚΑΤΩ. Έτσι η τομή περνά ακριβώς
   *  από τη μετρημένη κορυφή, χωρίς μετατόπιση της στάθμης — και τα τρίγωνα
   *  που απλώς ακουμπούν τη στάθμη σε μία κορυφή βγάζουν τμήμα μηδενικού
   *  μήκους, που πετιέται παρακάτω.
   */
  function crossing(A, B, C, z) {
    var v = [A, B, C], out = [];
    for (var k = 0; k < 3; k++) {
      var P = v[k], Q = v[(k + 1) % 3];
    ...
    // Μηδενικού μήκους: το τρίγωνο ακουμπά τη στάθμη σε ένα σημείο. Αν μείνει,
    // γίνεται βρόχος πάνω στον εαυτό του και κόβει τη γραμμή στα δύο.
    return Math.hypot(out[0].x - out[1].x, out[0].y - out[1].y) < 1e-6 ? null : out;
  }

  /** Πού τέμνει το τμήμα P→Q το τμήμα R→S; Επιστρέφει το «t» πάνω στο P→Q. */
  function segCross(P, Q, R, S) {
    var dx = Q.x - P.x, dy = Q.y - P.y, ex = S.x - R.x, ey = S.y - R.y;
    var den = dx * ey - dy * ex;
    if (Math.abs(den) < 1e-12) return null;                 // παράλληλα
    var t = ((R.x - P.x) * ey - (R.y - P.y) * ex) / den;
    var u = ((R.x - P.x) * dy - (R.y - P.y) * dx) / den;
    return (t > 0 && t < 1 && u >= 0 && u <= 1) ? t : null;
  }

  /**
   *  Κόβει το τμήμα στο πολύγωνο και κρατά μόνο ό,τι είναι μέσα.
   *
   *  Το τμήμα σπάει σε όλα τα σημεία που τέμνει το περίγραμμα, και κάθε
   *  κομμάτι κρίνεται από το ΜΕΣΟΝ του: έτσι δουλεύει και σε κοίλα πολύγωνα,
   *  όπου ένα τμήμα μπορεί να μπει και να βγει πολλές φορές.
   */
  function clipSeg(P, Q, ring) {
    var ts = [0, 1];
    for (var i = 0, n = ring.length, j = n - 1; i < n; j = i++) {
      var t = segCross(P, Q, ring[j], ring[i]);
      if (t != null) ts.push(t);
    }
    if (ts.length === 2) {
      var mx = (P.x + Q.x) / 2, my = (P.y + Q.y) / 2;
      return inPolygon(mx, my, ring, 0) ? [[P, Q]] : [];
    }
    ts.sort(function (a, b) { return a - b; });
    var at = function (t) { return { x: P.x + t * (Q.x - P.x), y: P.y + t * (Q.y - P.y) }; };
    var out = [];
    for (var k = 0; k < ts.length - 1; k++) {
      var t0 = ts[k], t1 = ts[k + 1];
      if (t1 - t0 < 1e-9) continue;
      var m = at((t0 + t1) / 2);
      if (inPolygon(m.x, m.y, ring, 0)) out.push([at(t0), at(t1)]);
    }
    return out;
  }

  /** Ενώνει τα κομμάτια σε συνεχείς γραμμές, από τα κοινά τους άκρα. */
  function chain(segs) {
    var key = function (p) { return Math.round(p.x * 1e4) + ',' + Math.round(p.y * 1e4); };
    var ends = {};
    segs.forEach(function (s, i) {
      [key(s[0]), key(s[1])].forEach(function (k) { (ends[k] || (ends[k] = [])).push(i); });
    });
    var used = new Array(segs.length), lines = [];
    for (var i = 0; i < segs.length; i++) {
      if (used[i]) continue;
      used[i] = 1;
      var line = [segs[i][0], segs[i][1]];
      // Πρώτα προς τα εμπρός, μετά αντιστροφή και πάλι προς τα εμπρός.
      for (var dir = 0; dir < 2; dir++) {
        for (;;) {
          var k = key(line[line.length - 1]), cand = ends[k] || [], nxt = -1;
          for (var j = 0; j < cand.length; j++) if (!used[cand[j]]) { nxt = cand[j]; break; }
          if (nxt < 0) break;
          used[nxt] = 1;
          var s = segs[nxt];
  ...
  /**
   *  Ισοϋψείς της επιφάνειας.
   *
   *  Κάθε τρίγωνο είναι επίπεδο, οπότε η τομή του με οριζόντιο επίπεδο είναι
   *  ένα ευθύγραμμο τμήμα — δεν χρειάζεται παρεμβολή σε κάνναβο ούτε εξομάλυνση.
   *  Οι ισοϋψείς που βγαίνουν περνούν ΑΚΡΙΒΩΣ από τα μετρημένα υψόμετρα.
   *
   *  Οι στάθμες μετατοπίζονται κατά ένα ελάχιστο, ώστε να μην πέφτει καμία
   *  ακριβώς πάνω σε κορυφή: εκεί η τομή θα ήταν σημείο αντί για τμήμα, και
   *  η γραμμή θα έσπαγε χωρίς λόγο.
   */
  function contours(points, opts) {
    opts = opts || {};
    var step = opts.interval;
    if (!(step > 0)) return { err: 'The contour interval must be positive.' };

    var m = mesh(points, opts.maxEdge);
    if (m.err) return m;
    if (!m.tris.length) return { err: 'No surface left to contour.' };

    var zmin = Infinity, zmax = -Infinity;
    points.forEach(function (p) { if (p.z < zmin) zmin = p.z; if (p.z > zmax) zmax = p.z; });
    var n = Math.floor((zmax - zmin) / step);
    if (n > 2000) return { err: 'Too many contours (' + n + '). Increase the interval.' };
    if (n < 1) return { err: 'The interval (' + step + ' m) is larger than the ' +
      'elevation range (' + (zmax - zmin).toFixed(2) + ' m).' };

    var ring = opts.ring && opts.ring.length >= 3 ? opts.ring : null;
    var out = [], total = 0;
    /*  Οι στάθμες μπαίνουν ΑΥΣΤΗΡΑ ανάμεσα στο ελάχιστο και στο μέγιστο.
     *  Ισοϋψής ακριβώς στο ένα άκρο είναι το χείλος των δεδομένων και όχι
     *  ισοϋψής: δεν λέει τίποτα για το ανάγλυφο, και όποια σύμβαση κι αν
     *  διαλέξει κανείς θα την κρατούσε στη μία άκρη και θα την έχανε στην άλλη. */
    var k0 = Math.floor(zmin / step) + 1;
    for (var i = k0; ; i++) {
      var z = i * step;
      if (z >= zmax - 1e-9) break;
      var segs = [];
      m.tris.forEach(function (t) {
        var s = crossing(t[0], t[1], t[2], z);
        if (!s) return;
        if (ring) clipSeg(s[0], s[1], ring).forEach(function (c) { segs.push(c); });
        else segs.push(s);
      });
      if (!segs.length) continue;
      var lines = chain(segs);
      lines.forEach(function (L) {
        for (var j = 1; j < L.length; j++) total += Math.hypot(L[j].x - L[j-1].x, L[j].y - L[j-1].y);
      });
      out.push({ z: z, lines: lines });
    }
    if (!out.length) return { err: 'No contours were produced.' };
    return { levels: out, interval: step, zmin: zmin, zmax: zmax,
             count: out.length, length: total, triangles: m.tris.length };
  }
  ...
  function compute(opts) {
    var ring = opts.ring || [];
    if (ring.length < 3) return { err: 'The polygon needs at least 3 vertices.' };
    var pts = (opts.points || []).slice();
    if (pts.length < 3) return { err: 'At least 3 points with elevation are needed.' };
    var h = opts.spacing;
    if (!(h > 0)) return { err: 'The grid spacing must be positive.' };
    var pts2 = opts.points2 || null;
    if (pts2 && pts2.length < 3) return { err: 'The second surface needs at least 3 points.' };
    if (!pts2 && !Number.isFinite(opts.refZ)) return { err: 'Give a final elevation or a second surface.' };

    var fa = farApart(pts, ring);
    if (fa.bad) return { err: 'The points and the polygon are ' + fa.dist.toFixed(0) +
      ' m apart — they are probably not in the same coordinate system.' };
    if (pts2) {
      var fa2 = farApart(pts2, ring);
      if (fa2.bad) return { err: 'The second surface is ' + fa2.dist.toFixed(0) +
        ' m from the polygon — probably a different coordinate system.' };
    }

    // Local origin: otherwise the squared coordinates lose precision.
    ...
    if (!sa.tris.length) return { err: 'The points are collinear — no surface can be formed.' };
    if (sb && !sb.tris.length) return { err: 'The second surface\'s points are collinear.' };
    ...
    // Positions come from an index, not by summing the step: on large
    // polygons repeated addition drifts by the last column.
    var nx = Math.ceil((maxx - (minx + h / 2)) / h);
    var ny = Math.ceil((maxy - (miny + h / 2)) / h);
    if (nx <= 0 || ny <= 0) return { err: 'The spacing is larger than the area.' };
    ...
    if (!nIn) return { err: 'No grid cell fell inside the polygon — reduce the spacing.' };
