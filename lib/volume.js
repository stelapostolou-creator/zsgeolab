...
(function (root) {
  ...
  function mesh(points, maxEdge) {
    ...
    var pts = points.map(function (p) { return { x: p.x - ox, y: p.y - oy, z: p.z }; });
    var all = triangulate(pts);
    if (!all.length) return { err: 'Τα σημεία είναι συνευθειακά — δεν σχηματίζεται επιφάνεια.' };
    var lim = (maxEdge > 0) ? maxEdge : Infinity;
    var out = [], dropped = 0;
  ...
  function contours(points, opts) {
    ...
    if (!(step > 0)) return { err: 'Το ισοδιάστημα πρέπει να είναι θετικό.' };

    var m = mesh(points, opts.maxEdge);
    if (m.err) return m;
    if (!m.tris.length) return { err: 'Δεν έμεινε επιφάνεια για ισοϋψείς.' };

    var zmin = Infinity, zmax = -Infinity;
    points.forEach(function (p) { if (p.z < zmin) zmin = p.z; if (p.z > zmax) zmax = p.z; });
    var n = Math.floor((zmax - zmin) / step);
    if (n > 2000) return { err: 'Πολλές ισοϋψείς (' + n + '). Μεγάλωσε το ισοδιάστημα.' };
    if (n < 1) return { err: 'Το ισοδιάστημα (' + step + ' m) είναι μεγαλύτερο από τη ' +
      'διαφορά υψομέτρων (' + (zmax - zmin).toFixed(2) + ' m).' };
    ...
    if (!out.length) return { err: 'Δεν προέκυψε καμία ισοϋψής.' };
