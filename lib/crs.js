 ...
 *
 * isReady() is false until a CRS has been explicitly set (by the caller, or
 * because the loaded file declared one) — the WGS84 literal below is only
 * the inert pre-choice placeholder, never a silent "good enough" default.
 * A tool built on this module should treat "not ready" as "ask the user for
 * an EPSG code", not fall through to showing raw lat/lon as if it were X/Y.
 */
(function (global) {
  'use strict';
  ...
  var ready = false;
  var pending = false;   // true only while an epsg.io lookup is actually in flight
  var listeners = [];
  ...
  function applyEPSG(code, onReady, onError) {
    ...
    var readyBefore = ready;   // whether a CRS was already explicitly chosen before this attempt
    ready = false;
    pending = true;
    fetch('https://epsg.io/' + code + '.proj4').then(function (r) {
      ...
      pending = false;
      ready = readyBefore; // restore prior readiness — nothing actually changed
  ...
  var API = {
    ...
    setState: setState, getState: getState, isReady: isReady, isPending: isPending, onChange: onChange,
